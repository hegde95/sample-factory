...
from plots.plot_utils import GREEN, BLUE, ORANGE, DARK_GREY,  set_matplotlib_params
...
# python -m plots.experiments.mean_std_plots_grid_sound_instruction_envs --path train_dir/doom_sound_instruction/doom_sound_instruction_ --env doom_once_sound_instruction
...
ENVS_LIST = [
    ('doom_sound_instruction', 'once'),
    ('doom_once_sound_instruction', None),
]

PLOT_NAMES = dict(
    doom_music_sound_multi='Find Music source',
)
...
def aggregate(enc, experiments, count, ax):
    print("Started aggregation {}".format(enc))

    curr_dir = os.path.dirname(os.path.abspath(__file__))
    cache_dir = join(curr_dir, 'cache')
    cache_enc = join(cache_dir, enc)

    if CACHE:
        if os.path.isdir(cache_enc):
            with open(join(cache_enc, f'{enc}.pickle'), 'rb') as fobj:
                interpolated_keys = pickle.load(fobj)
        else:
            cache_enc = ensure_dir_exists(cache_enc)
...
def plot(env, key, interpolated_key, ax, count):
    ...
    y_plus_std = np.clip(y_plus_std, 0,1)
    ...
    ax.set_ylim(ymin=min(np.min(y_mean) - y_delta, 0.0), ymax=np.max(y_plus_std) + y_delta)
    # plt.grid(False)
    ...
    sf_plot, = ax.plot(x, y_mean, color=ENCODERS_COLOR[env], label=ENCODERS_NAMES[env], linewidth=lw, antialiased=True)
    ax.fill_between(x, y_minus_std, y_plus_std, color=ENCODERS_COLOR[env], alpha=0.25, antialiased=True, linewidth=0.0)
